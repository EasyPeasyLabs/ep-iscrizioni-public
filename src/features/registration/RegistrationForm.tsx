...
// -- HELPERS --
const computeAgeInMonths = (dobString?: string): number => {
...
const dayNumberMap: { [key: number]: string } = {
  1: 'Lunedì', 2: 'Martedì', 3: 'Mercoledì', 4: 'Giovedì', 5: 'Venerdì', 6: 'Sabato', 7: 'Domenica', 0: 'Domenica'
};
...
export const RegistrationForm: React.FC<RegistrationFormProps> = ({ onProgressUpdate, onSuccess }) => {
  const [formData, setFormData] = useState({
    nome: '', cognome: '', email: '', telefono: '', childName: '', childDob: '', selectedLocation: '', selectedSlot: ''
  });
  ...
  const handleScroll = () => resetScrollTimer();
  ...
  useEffect(() => {
    const fetchPortalTexts = async () => {
      try {
        const response = await fetch('/api/getPortalTexts', { method: 'GET', headers: { 'Accept': 'application/json' } });
        if (response.ok) {
          const apiResponse = await response.json();
          if (apiResponse.success && Array.isArray(apiResponse.data)) setPortalTexts(apiResponse.data);
        }
      } catch (error) { console.warn('Warning fetching portal texts:', error); }
    };
    fetchPortalTexts();
  }, []);
  ...
  const locationsWithRealAvailability = React.useMemo(() => {
    return availableLocations.map(loc => ({
      ...
      bundles: loc.bundles.map(bundle => {
        ...
        return { ...bundle, availableSeats: realAvailableSeats, isFull: realAvailableSeats === 0 };
      })
    }));
  ...
  useEffect(() => {
    let frame = 0;
    if (loading) frame = 6;
    else if (currentCard === 4) frame = privacyAccepted ? 5 : 4;
    else frame = currentCard;
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (id === 'selectedLocation') setFormData(prev => ({ ...prev, selectedLocation: value, selectedSlot: '' }));
    else if (id === 'telefono') {
      let val = value.replace(/[^\d+]/g, '');
      const oldVal = formData.telefono;
      if (val.length > 0) {
        if (val.startsWith('0039')) val = '+39' + val.substring(4);
        else if (!val.startsWith('+39')) {
          val = val.replace(/^\+?/, '');
          if (oldVal === '+39' && val.length < 3) val = '';
          else if (val.startsWith('39') && val.length > 3) val = '+' + val;
          else val = '+39' + val;
        }
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const leadData = {
        ...
        selectedSlot: selectedBundle ? {
          bundleId: selectedBundle.bundleId,
          subscriptionId: selectedBundle.subscriptionId,
          bundleName: selectedBundle.publicName || selectedBundle.name,
          ...
          type: selectedBundle.includedSlots.length === 1 ? selectedBundle.includedSlots[0].type : selectedBundle.includedSlots.map(s => s.type).join("+"),
          label: `${dayNumberMap[selectedBundle.dayOfWeek]} ${selectedBundle.includedSlots.map(s => s.startTime).join(" & ")}`
        } : formData.selectedSlot,
        notes: `Selected Bundle: ${selectedBundle?.publicName}. Lead from Public Landing Page (E2E Aligned)`,
        status: "pending",
        privacyConsent: true,
      ...
      if (db) await addDoc(collection(db, "raw_registrations"), leadData);
      else throw new Error("Database connection not initialized");

      if (onSuccess) {
        onSuccess();
        setTimeout(() => {
          setFormData({ nome: '', cognome: '', email: '', telefono: '', childName: '', childDob: '', selectedLocation: '', selectedSlot: '' });
  ...
  const childAgeMonths = computeAgeInMonths(formData.childDob);
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Enter') {
      ...
      else if (isCardValid(currentCard)) handleSubmit(e as unknown as React.FormEvent);
    }
  };
  ...
  const renderCardContent = (index: number) => {
    ...
    if (hasAbsenceTexts) { absenceStepIndex = 4; if (index >= absenceStepIndex) effectiveIndex--; }
    if (hasPaymentTexts) { paymentStepIndex = hasAbsenceTexts ? 5 : 4; if (index >= paymentStepIndex) effectiveIndex--; }
          ...
          <div className="max-h-[300px] overflow-y-auto space-y-3">
            {paymentTexts.map(text => (
              <div key={text.id} className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <h4 className="text-sm font-bold text-blue-800 mb-2">{text.paymentMethod ? `${text.paymentMethod}: ${text.title}` : text.title}</h4>
            ...
            <div className="flex items-baseline justify-start gap-4 border-b border-slate-100 pb-1 mb-2">
              ...
              {childAgeMonths > 0 && (
                <p className="text-[11px] text-slate-400">
                  Mostra corsi per età: <span className="font-black text-slate-600 text-[14px]">{childAgeYears}</span> <span className="font-bold text-slate-400">anni</span>
                </p>
              )}
            </div>

            <div className="flex flex-col gap-1 h-full relative">
              <div ref={scrollContainerRef} onScroll={handleScroll} onTouchStart={resetScrollTimer} className="flex flex-col gap-5 w-full max-h-[420px] overflow-y-auto pr-0.5 scroll-smooth pb-10">
                {isLoadingLocations ? (
                  ...
                  filteredLocations.map(loc => {
                    ...
                    if (!city && loc.indirizzo) {
                      ...
                      city = parts.length > 1 ? parts[parts.length - 1].trim().toUpperCase() : "CITTÀ";
                      ...
                      <div key={loc.sedeId} className="space-y-4 mb-8">
                        <div className="flex gap-2 h-20 px-1 items-stretch">
                          <div className="flex-grow bg-[#fdf289] border-[3px] border-slate-300 rounded-[1.5rem] flex items-center p-3 gap-3 shadow-sm relative min-w-0">
                            <div className="bg-white border-[3px] border-slate-300 rounded-xl p-1.5 flex-shrink-0 shadow-inner"><Home className="w-8 h-8 text-slate-800" strokeWidth={2.5} /></div>
                            <div className="flex flex-col leading-none overflow-hidden pr-10"><span className="font-black text-slate-800 text-[14px] sm:text-[15px] uppercase break-words tracking-tight">{loc.nomeSede}</span></div>
                            <div className="absolute bottom-2 right-4 leading-none"><span className="font-bold text-slate-600 text-[10px] uppercase italic">({city})</span></div>
                          </div>
                          {loc.indirizzo && (
                            <a href={loc.googleMapsLink || `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(loc.indirizzo)}`} target="_blank" rel="noopener noreferrer" className="w-20 flex-shrink-0 bg-[#9c1b1c] border-[3px] border-slate-300 rounded-[1.5rem] flex flex-col items-center justify-center text-white transition-all active:scale-95 shadow-md hover:bg-red-900" onClick={(e) => e.stopPropagation()}>
                              <div className="bg-white rounded-full p-1 mb-1 shadow-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9c1b1c" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 10c0 6-9 13-9 13s-9-7-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                              </div>
                        ...
                        <div className="space-y-5 px-1">
                          {visibleBundles.map((bundle) => {
                            ...
                            const min = bundle.minAge; const max = bundle.maxAge;
                            const dayName = dayNumberMap[bundle.dayOfWeek] || 'Sconosciuto';
                            const isFull = bundle.isFull || bundle.availableSeats === 0;
                            return (
                              <div key={bundle.bundleId} className={`relative p-5 rounded-[2.2rem] border-[3px] transition-all duration-300 bg-white shadow-xl ${isFull ? 'opacity-60 cursor-not-allowed border-slate-100' : ''} ${isSelected && !isFull ? 'border-brand-blue ring-4 ring-blue-100 scale-[1.01]' : !isFull ? 'border-slate-100' : ''}`}>
                                <div className="flex items-center justify-between mb-4">
                                  <h4 className="font-black text-[#1d4f91] text-[22px] tracking-tight leading-none uppercase">{bundle.publicName}</h4>
                                  {bundle.description && (
                                    <button type="button" onClick={(e) => { e.stopPropagation(); setInfoModalContent({ title: bundle.publicName || bundle.name, description: bundle.description || '' }); }} className="w-10 h-10 rounded-full border-[3px] border-brand-blue flex items-center justify-center text-brand-blue bg-white shadow-sm hover:bg-blue-50 transition-colors">
                                      <span className="font-serif italic font-black text-[22px] leading-none">i</span>
                                    </button>
                                  )}
                                </div>
                                <div className="flex items-center justify-between mb-5">
                                  <div className="px-5 py-1.5 border-[3px] border-brand-blue rounded-full bg-white shadow-sm"><span className="text-brand-blue font-black text-[12px] uppercase tracking-wider">{dayName}</span></div>
                                  <button type="button" onClick={(e) => { e.stopPropagation(); if (isChildDobValid && !isFull) setFormData(prev => ({ ...prev, selectedLocation: loc.sedeId, selectedSlot: bundle.bundleId })); }} className={`px-8 py-1.5 border-[3px] rounded-2xl font-black text-[14px] uppercase transition-all ${isSelected ? 'bg-green-600 border-green-600 text-white shadow-md' : 'bg-white border-[#22c55e] text-[#22c55e] hover:bg-green-50'}`}>
                                ...
                                <div className="flex gap-3 items-stretch">
                                      ...
                                      return (
                                        <div key={idx} className="flex flex-col gap-1.5">
                                          <div className="flex items-center gap-3">
                                            <span className="bg-blue-100 text-brand-blue px-2 py-0.5 rounded text-[10px] font-black uppercase shadow-sm">{slot.type}</span>
                                            <span className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">{(dayNumberMap[bundle.dayOfWeek] || 'LUN').substring(0,3).toUpperCase()}</span>
                                          </div>
                                  ...
                                  {(() => {
                                    ...
                                    let bg = '#32CD32'; let tc = '#171717'; let tt = 'POSTI DISPONIBILI:'; let bt = seats.toString();
                                    if (seats === 0) { bg = '#C0C0C0'; tt = 'POSTI ESAURITI'; bt = ''; }
                                    else if (occupied <= 1) { bg = '#017010'; tc = '#ffffff'; tt = 'Nuovo corso in partenza!'; bt = ''; }
                                    else if (seats <= 2) bg = '#FFFF00';
                                    return (
                                      <div className="flex-1 rounded-2xl flex flex-col items-center justify-center p-2 text-center shadow-lg border-b-4 border-black/10 min-w-[100px]" style={{ backgroundColor: bg, color: tc }}>
                                        <div className="text-[9px] font-black leading-tight uppercase opacity-90 px-1 tracking-tighter">{tt}</div>
                                        {bt && <div className="text-[36px] font-black leading-none mt-1">{bt}</div>}
                                      </div>
                                    );
                                  })()}
                                </div>
                        ...
                        <div className="pt-6 pb-2 px-10"><div className="h-[1px] w-full bg-slate-200 shadow-sm" /></div>
                      </div>
                    );
                  })
                )}
                ...
                <div className="absolute bottom-2 left-1/2 -translate-x-1/2 z-20 pointer-events-none">
                  <div className="flex flex-col items-center">
                    ...
                    <div className="animate-bounce">
                      <svg width="24" height="20" viewBox="0 0 24 24" fill="none" stroke="#2b71b8" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" style={{ animation: 'pulse-v 1.5s infinite ease-in-out' }}><path d="M7 13l5 5 5-5M7 6l5 5 5-5" /></svg>
                    </div>
                  </div>
                </div>
              )}
              ...
              <style>{`@keyframes pulse-v { 0%, 100% { transform: translateY(0) scale(1); opacity: 0.8; } 50% { transform: translateY(8px) scale(1.1); opacity: 1; } }`}</style>
              {firstAvailableDate && (
          ...
          <div className="space-y-2">
            ...
            <div className={`transition-opacity duration-300 ${!isSlotValid ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
              <div className="flex items-start gap-2">
                <div className="flex items-center h-5">
                  <input id="privacy" name="privacy" type="checkbox" checked={privacyAccepted} disabled={!isSlotValid} onChange={(e) => { setPrivacyAccepted(e.target.checked); if (e.target.checked && errors.privacy) setErrors(prev => ({ ...prev, privacy: undefined })); }} className={`h-4 w-4 rounded border-gray-300 text-brand-blue focus:ring-brand-blue cursor-pointer ${errors.privacy ? 'border-red-300 ring-1 ring-red-300' : ''}`} />
                </div>
            ...
            {globalError && (
              <div className="p-2 rounded-xl bg-red-50 border border-red-100 animate-pulse">
                <div className="flex"><div className="flex-shrink-0"><svg className="h-4 w-4 text-red-400" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clipRule="evenodd" /></svg></div>
                  <div className="ml-2"><h3 className="text-xs font-medium text-brand-red">Errore</h3><div className="mt-0.5 text-xs text-red-700"><p>{globalError}</p></div></div>
                </div>
              </div>
            )}
            <div className={`pt-1 transition-all duration-300 ${!privacyAccepted ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
              <Button type="button" onClick={handleSubmit} isLoading={loading} disabled={!privacyAccepted} className="w-full bg-brand-red hover:bg-red-700 text-white font-bold py-2 px-4 rounded-xl shadow-md transition-all duration-300 font-sans text-base">INVIA</Button>
  ...
  return (
    <>
          ...
          <form onKeyDown={handleKeyDown} className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentCard * 100}%)` }}>
            {Array.from({ length: totalCards }, (_, i) => i).map((index) => (
              <div key={index} className="w-full flex-shrink-0 px-1">
                <Card className="shadow-2xl border-0 rounded-3xl bg-white/95 backdrop-blur-sm pt-3 border-t-4 border-brand-red min-h-[200px] flex flex-col"><CardContent className="px-4 pb-1 pt-1 flex-1 overflow-y-auto">{renderCardContent(index)}</CardContent></Card>
              </div>
            ))}
          </form>
        </div>
      ...
      <div className="flex justify-center gap-2 -mt-6 pb-4">
        {Array.from({ length: totalCards }, (_, i) => i).map((index) => (
          <div key={index} className={`h-2 rounded-full transition-all duration-300 ${currentCard === index ? 'w-6 bg-brand-blue' : 'w-2 bg-slate-300'}`} />
        ))}
      </div>
      <Modal isOpen={showPrivacyModal} onClose={() => setShowPrivacyModal(false)} title="Informativa sulla Privacy">
        <div className="space-y-4 text-slate-600 font-serif text-sm leading-relaxed">
          ...
          <p>I dati raccolti saranno utilizzati esclusivamente per l'organizzazione dell'evento, l'invio di comunicazioni logistiche e la gestione degli accessi.</p>
          <h4 className="font-bold text-brand-blue font-serif text-base">2. Conservazione dei dati</h4>
          <p>I tuoi dati saranno conservati per il tempo strettamente necessario all'espletamento delle finalità sopra indicate.</p>
          <h4 className="font-bold text-brand-blue font-serif text-base">3. I tuoi diritti</h4>
          <p>Hai il diritto di chiedere l'accesso ai tuoi dati personali, la rettifica, la cancellazione degli stessi o la limitazione del trattamento.</p>
      ...
      {infoModalContent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-6">
              <h3 className="text-xl font-extrabold text-brand-blue mb-3">{infoModalContent.title}</h3>
              <div className="text-sm text-slate-600 leading-relaxed mb-6 whitespace-pre-wrap">{infoModalContent.description}</div>
              <div className="flex justify-end"><button type="button" onClick={() => setInfoModalContent(null)} className="px-5 py-2 bg-brand-blue text-white font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-sm">Ho capito</button></div>
            </div>
          </div>
        </div>
      )}
